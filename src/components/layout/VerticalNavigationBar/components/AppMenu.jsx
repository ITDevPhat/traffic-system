...
const MenuItem = ({
  ...
  return <li className={className}>
      {isClickable ? <MenuItemLink item={item} className={linkClassName} /> : <div className={clsx(linkClassName, 'disabled')} role="button" aria-disabled>
          {item.icon && <span className="nav-icon">
              <IconifyIcon icon={item.icon} />
            </span>}
          <span className="nav-text">{item.label}</span>
...
const MenuItemLink = ({
  ...
  const href = item.url || '#';
  const isDisabled = item.isDisabled || href === '#';
  return <Link href={href} target={item.target} className={clsx(className, { disabled: isDisabled })} onClick={e => {
...
const AppMenu = ({
  ...
  const activeMenu = useCallback(() => {
    ...
    const trimmedURL = normalize(pathname);
