...
export default function EditModelPage() {
  ...
  const { control, handleSubmit, reset } = useForm<ModelUpdateInput>({
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const errorMsg = err.message || 'Không thể tải thông tin mô hình';
        setError(errorMsg);
  ...
  const onSubmit = async (data: ModelUpdateInput) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const errorMsg = err.message || 'Không thể cập nhật mô hình';
      setError(errorMsg);
      toast.error(errorMsg);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <PageTitle title="Chỉnh sửa mô hình AI" subName="Cập nhật" />

      <Row>
        <Col lg={8} className="mx-auto">
          <Card className="shadow-sm">
            <Card.Header className="bg-white py-3">
              <h5 className="mb-0">✏️ Chỉnh sửa thông tin mô hình</h5>
            </Card.Header>
